...
import core.util.BitcoinCashBase32;
...
public class AddressUtil {
	...
	public static String toCashAddress(byte addressType, byte[] hash) {
		String prefixString = Constants.TEST_NET ? Constants.TESTNET_PREFIX : Constants.MAINNET_PREFIX;
		...
		return prefixString + Constants.SEPARATOR_COLON + cashAddress;
	}
...
//	public static String to_scripthash_hex_from_testAddr(String address){
//		Storage storage = null;
//		try {
//			storage = new Storage(Constants.STORAGE_PATH, PassWordActivity.password);
...
//			int reciveIndex = getIndex(receivingArray, address);
//			int changeIndex = getIndex(changeArray, address);
...
//			if (reciveIndex >= 0) {
//				String[] changeAddresses = addressUtil.generateLegacyAddresses(1, AddressUtil.ADDRESS_TYPE_RECEIVING);
//				return to_scripthash_hex_from_legacy(changeAddresses[0]);
//			} else if (changeIndex >= 0) {
//				String[] reciveAddresses = addressUtil.generateLegacyAddresses(1, AddressUtil.ADDRESS_TYPE_CHANGE);
//				return to_scripthash_hex_from_legacy(reciveAddresses[0]);
//			}
...
//	}
	...
	public static String generateCashAddrFromLegacyAddr(String legacyAddr) {
		...
		String prefixString = Constants.TEST_NET ? Constants.TESTNET_PREFIX : Constants.MAINNET_PREFIX;
		return toCashAddress(P2PKH, address).substring((prefixString + Constants.SEPARATOR_COLON).length());
	}
	...
	public String[] generateTestAddresses(String[] pubKeyHash) {
		String[] addresses = new String[pubKeyHash.length];
		for (int i = 0; i < pubKeyHash.length; i++) {
			byte[] bytes = Util.hexStringToByteArray(pubKeyHash[i]);
			addresses[i] = toCashAddress(P2PKH, bytes);
		}
		return addresses;
	}
	...
	public static boolean isValidCashAddress(String bitcoinCashAddress) {
		try {
			String prefix = Constants.TEST_NET ? Constants.TESTNET_PREFIX : Constants.MAINNET_PREFIX;
	...
	public static String testAddrfromPubKeyHash(String pubKeyHash) {

		BigInteger bi_58 = new BigInteger("58");
		String b58chars = Constants.BASE_58_CHARS;

		StringBuffer addr = new StringBuffer("6f").append(pubKeyHash);
		String checksum = Bitcoin.Hash(addr.toString());
		String checksumhead = (checksum.substring(0, 8));
		addr.append(checksumhead);
		BigInteger int_payload = new BigInteger(addr.toString(), 16);
