...
public class Transaction {
	...
	private static final String NVERSION = "02000000";
	private static final String NSEQUENCE = "ffffffff";
//	private static final String NSEQUENCE = "feffffff";
	...
	public static String serializePreimage(int i, BigInteger localBlockHeight, int prevout_n, TxIn[] txins,
		...
		if (addr.startsWith("2") || addr.startsWith("3")) {
			preimage_script = P2SH_script(decode(addr));
		}
	...
	public static String serializeOutput(TxOut txout) {
		...
		String script;
		// Whc logic
		if (!TextUtils.isEmpty(txout.getRawScript())) {
			script = txout.getRawScript();
		} else
		if (addr.length() > 35) {
			String prefixString = Constants.TEST_NET ? Constants.TESTNET_PREFIX : Constants.MAINNET_PREFIX;
			if (!addr.startsWith(prefixString)) {
				addr = prefixString + Constants.SEPARATOR_COLON + addr;
			}
			BitcoinCashAddressDecodedParts decP = AddressUtil.decodeCashAddress(addr);
			byte[] p2pkh = decP.getHash();
			script = Util.bytesToHex(p2pkh);
			if (AddressUtil.P2SH == decP.addressType) {
				script = P2SH_script(script);
			} else {
			...
			if (addr.startsWith("2") || addr.startsWith("3")) {
				script = P2SH_script(decode(addr));
			} else  {
	...
	public static String P2SH_script(String hash160) {

		String OP_HASH160 = "a9";
		String OP_EQUAL = "87";
		String push_data = push_data(hash160);
		String retval = OP_HASH160 + push_data + OP_EQUAL;
		return retval;
